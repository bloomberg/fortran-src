...
main = do
  ...
  mapM_ (printTag path) (allTags prog)
  return ()
...
allTags :: ProgramFile A0 -> [Tag]
allTags = everything (++) (mkQ [] programUnitTag `extQ` commonGroupTags)

programUnitTag :: ProgramUnit A0 -> [Tag]
programUnitTag pu = case pu of
...
statementTag :: Statement A0 -> [Tag]
statementTag st = case st of
  -- StDeclaration _ s _ _ ds -> undefined
...
dataGroupTags :: DataGroup A0 -> [Tag]
...
commonGroupTags :: CommonGroup A0 -> [Tag]
...
varTag :: TagKind -> Expression A0 -> Maybe Tag
varTag k e = case e of
  ExpValue _ s (ValVariable n) -> Just (Tag s k n)
