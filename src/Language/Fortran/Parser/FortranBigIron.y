...
PROGRAM_INNER
: PROGRAM_UNITS { ProgramFile (MetaInfo { miVersion = FortranBigIron, miFilename = "" }) (reverse $1) }

PROGRAM_UNITS :: { [ ProgramUnit A0 ] }
PROGRAM_UNITS
: PROGRAM_UNITS MAYBE_LABEL_IN_6COLUMN PROGRAM_UNIT MAYBE_NEWLINE { $3 : $1 }
| MAYBE_LABEL_IN_6COLUMN PROGRAM_UNIT MAYBE_NEWLINE { [ $2 ] }

MAYBE_LABEL_IN_6COLUMN :: { Maybe (Expression A0) }
MAYBE_LABEL_IN_6COLUMN
: LABEL_IN_6COLUMN { Just $1 }
| {- empty -}      { Nothing }
...
| goto VARIABLE { StGotoAssigned () (getTransSpan $1 $2) $2 (fromList () []) }
...
bigIronParser = bigIronParserWithModFiles emptyModFiles

bigIronParserWithModFiles ::
    ModFiles -> B.ByteString -> String -> ParseResult AlexInput Token (ProgramFile A0)
bigIronParserWithModFiles mods sourceCode filename =
